# Documentação: Encarregado - Gestão (Index)
...
A página **Encarregado - Index** permite visualizar, gerenciar e controlar o status de todos os encarregados cadastrados no sistema. Encarregados são pessoas responsáveis por supervisionar contratos e operações de veículos. A página oferece:

- ✅ **Listagem completa** de encarregados com informações essenciais (Nome, Ponto, Celular, Contrato, Status)
...
1. **Pages/Encarregado/Index.cshtml** - View Razor da página (inclui modal de foto)
2. **Pages/Encarregado/Index.cshtml.cs** - PageModel (backend da página)
3. **wwwroot/js/cadastros/encarregado.js** - Lógica JavaScript do DataTable e ações
4. **Controllers/EncarregadoController.cs** - Endpoints API REST

---

## 1. Pages/Encarregado/Index.cshtml

### Problema
Criar uma interface visual clara e moderna para listar encarregados, com badges de status clicáveis, botões de ação padronizados, e modal para visualização de fotos.
...
<div class="ftx-card-header">
    <h2 class="titulo-paginas">
        ...
        Listagem de Encarregados
    </h2>
    <a href="/Encarregado/Upsert" class="btn btn-fundo-laranja" data-ftx-loading>
...
<div class="panel-content">
    <div id="divEncarregados">
        <table id="tblEncarregado" class="table table-bordered table-striped" width="100%">
            <thead>
                <tr>
                    <th>Nome</th>
                    <th>Ponto</th>
    ...
    try {
        ...
        $(document).on('click', '.btn-foto', function(e) {
            try {
                ...
                var nomeEncarregado = $(this).closest('tr').find('td:eq(0)').text();
                ...
                $.ajax({
                    type: "GET",
                    url: "/api/Encarregado/PegaFotoModal",
                    data: { id: encarregadoId },
                    success: function (res) {
                        try {
                            if (res === false || !res) {
                                $('#txtTituloFoto').html('Encarregado sem Foto: <b>' + nomeEncarregado + '</b>');
                                $('#imgViewer').attr('src', '/Images/barbudo.jpg');
                            } else {
                                $('#txtTituloFoto').html('Foto: <b>' + nomeEncarregado + '</b>');
                                $('#imgViewer').attr('src', 'data:image/jpg;base64,' + res);
                            }
                        } catch (error) {
                            Alerta.TratamentoErroComLinha("Encarregado/Index.cshtml", "PegaFotoModal.success", error);
                        }
                    },
                    error: function (err) {
                        try {
                            ...
                            Alerta.TratamentoErroComLinha("Encarregado/Index.cshtml", "PegaFotoModal.error", error);
                        }
                    }
                });
            } catch (error) {
                Alerta.TratamentoErroComLinha("Encarregado/Index.cshtml", "btn-foto.click", error);
            }
        });
        ...
        $('#modalFoto').on('hide.bs.modal', function () {
            try {
                $('#imgViewer').attr('src', '/Images/barbudo.jpg');
                $('#txtTituloFoto').text('Foto do Encarregado');
            } catch (error) {
                Alerta.TratamentoErroComLinha("Encarregado/Index.cshtml", "modalFoto.hide", error);
            }
        });
    } catch (error) {
        Alerta.TratamentoErroComLinha("Encarregado/Index.cshtml", "document.ready", error);
    }
});
```
...
## 2. Pages/Encarregado/Index.cshtml.cs
    ...
    public class IndexModel : PageModel
    {
        public void OnGet()
        {
            try
            {
                ...
                // O DataTable carrega dados via AJAX da API /api/encarregado
            }
...
## 3. wwwroot/js/cadastros/encarregado.js
    ...
    try
    {
        dataTable = $("#tblEncarregado").DataTable({
            ...
            columnDefs: [
                ...
                { targets: 3, className: "text-left", width: "29%" }, // Contrato
                { targets: 4, className: "text-center", width: "10%" }, // Status
                { targets: 5, className: "text-center", width: "16%" } // Ação
            ],
            responsive: true,
            ajax: {
                url: "/api/encarregado", // ✅ Endpoint API
                type: "GET",
                datatype: "json"
            },
            columns: [
                ...
                { data: "ponto" },
                { data: "celular01" },
                { data: "contratoEncarregado" }, // ✅ Formato: "Ano/Numero - Fornecedor"
                {
...
**Problema**: Ao clicar no botão de excluir, precisamos confirmar a ação do usuário antes de enviar a requisição à API. Além disso, o backend deve validar se existem contratos associados ao encarregado.

**Solução**: Usar SweetAlert (`Alerta.Confirmar`) para solicitar confirmação, e só então fazer a chamada AJAX POST para `/api/Encarregado/Delete`. O backend valida dependências antes de excluir.
...
**Problema**: Quando o usuário clica no badge de status (Ativo/Inativo), precisamos alternar o status do encarregado e atualizar a visualização do badge sem recarregar a página inteira.

**Solução**: Fazer chamada AJAX GET para `/api/Encarregado/UpdateStatusEncarregado`, e atualizar dinamicamente as classes CSS e o conteúdo HTML do badge baseado na resposta (`data.type`).
...
## 4. Controllers/EncarregadoController.cs

### Problema 1: Retornar lista de encarregados com informações de contrato e fornecedor
...
**Solução**: Fazer JOINs entre `Encarregado`, `Contrato` e `Fornecedor` usando LEFT JOIN para permitir encarregados sem contrato, e formatar o campo `ContratoEncarregado` como "Ano/Numero - Fornecedor".
...
public IActionResult Get()
{
    try
    {
        // ✅ JOINs complexos: Encarregado → Contrato → Fornecedor
        var result = (
...
### Problema 2: Excluir encarregado com validação de dependências
**Problema**: Ao excluir um encarregado, precisamos validar se existem contratos associados a ele através da tabela `EncarregadoContrato`. Se houver, a exclusão deve ser bloqueada.

**Solução**: Buscar o encarregado pelo ID, verificar se existe algum registro em `EncarregadoContrato` com `EncarregadoId` igual ao ID do encarregado. Se existir, retornar erro. Caso contrário, excluir o encarregado.
...
### Problema 3: Alternar status do encarregado
**Problema**: Ao clicar no badge de status, precisamos inverter o valor booleano `Status` do encarregado e retornar informações sobre a mudança (tipo 0=Ativo, 1=Inativo) para atualizar a interface.

**Solução**: Buscar o encarregado, inverter `Status`, atualizar no banco, e retornar JSON com `success`, `message` e `type`.
...
### Problema 4: Retornar foto do encarregado para modal
**Problema**: Quando o usuário clica no botão de foto, precisamos retornar a foto do encarregado em formato Base64 para exibir no modal.

**Solução**: Buscar o encarregado pelo ID, verificar se tem foto (`Foto != null`), converter para Base64, e retornar como JSON. Se não tiver foto, retornar `false`.
...
1. **Usuário acessa** `/Encarregado` ou `/Encarregado/Index`
2. **Servidor renderiza** `Pages/Encarregado/Index.cshtml` (PageModel `OnGet()` não carrega dados)
3. **Browser carrega** `encarregado.js` via `@section ScriptsBlock`
4. **JavaScript executa** `$(document).ready()` → chama `loadList()`
5. **DataTable inicializa** e faz requisição AJAX GET para `/api/encarregado`
6. **Controller `Get()`** executa JOINs complexos entre `Encarregado`, `Contrato` e `Fornecedor`
7. **Controller formata** campo `ContratoEncarregado` como "Ano/Numero - Fornecedor" ou "(Sem Contrato)"
...
10. **Badges de status** são renderizados como elementos clicáveis com classes CSS `updateStatusEncarregado`
...
3. **JavaScript extrai** `data-id` do botão e nome do encarregado da linha da tabela
...
6. **AJAX GET** é enviado para `/api/Encarregado/PegaFotoModal?id={guid}`
7. **Controller `PegaFotoModal()`** busca o encarregado pelo ID
...
12. **Título do modal** é atualizado com nome do encarregado

### Fluxo 3: Alternar Status (Clique no Badge)

1. **Usuário clica** no badge de status (Ativo ou Inativo)
2. **Event handler** `.updateStatusEncarregado` captura o clique
3. **JavaScript extrai** `data-url` do elemento (ex: `/api/Encarregado/UpdateStatusEncarregado?Id={guid}`)
4. **AJAX GET** é enviado para a URL
5. **Controller `UpdateStatusEncarregado()`** busca o encarregado pelo ID
...
5. **Se usuário confirma** (`willDelete === true`):
   - JavaScript monta objeto JSON `{ EncarregadoId: id }`
   - **AJAX POST** é enviado para `/api/Encarregado/Delete`
   - **Controller `Delete()`** valida o modelo e busca o encarregado
   - **Controller verifica** se existe `EncarregadoContrato` associado
...
### Fluxo 5: Editar Encarregado

1. **Usuário clica** no botão de editar (ícone de lápis)
2. **Link HTML** redireciona para `/Encarregado/Upsert?id={guid}`
...
|--------|----------|-----------|------------|---------|
| **GET** | `/api/encarregado` | Lista todos os encarregados para DataTable (com JOINs) | Nenhum | `{ data: [{ encarregadoId, nome, ponto, celular01, contratoEncarregado, status, foto, ... }, ...] }` |
| **POST** | `/api/Encarregado/Delete` | Exclui um encarregado (com validação de contratos) | Body: `{ EncarregadoId: Guid }` | `{ success: bool, message: string }` |
| **GET** | `/api/Encarregado/UpdateStatusEncarregado` | Alterna status (Ativo ↔ Inativo) | Query: `Id={Guid}` | `{ success: bool, message: string, type: int }` (type: 0=Ativo, 1=Inativo) |
| **GET** | `/api/Encarregado/PegaFotoModal` | Retorna foto do encarregado em Base64 | Query: `id={Guid}` | `string` (Base64) ou `false` |
...
**Causa**: O método `UpdateStatusEncarregado` no Controller não chama `_unitOfWork.Save()` após `Update()`, então a alteração não é persistida no banco de dados.

**Solução**: 
- Adicionar `_unitOfWork.Save();` após `_unitOfWork.Encarregado.Update(objFromDb);` na linha 178 do `EncarregadoController.cs`
...
1. Encarregado não tem foto cadastrada (`Foto` é `null`)
2. Erro na conversão Base64
3. Erro na requisição AJAX

**Solução**:
- Verificar se o encarregado tem foto no banco de dados
- Abrir Console do navegador (F12) e verificar erros JavaScript
- Verificar Network tab para ver resposta da requisição `/api/Encarregado/PegaFotoModal`
- Verificar logs do servidor para erros no Controller

### Problema: Não consegue excluir encarregado
**Causa**: Encarregado está associado a um ou mais contratos através da tabela `EncarregadoContrato`.

**Solução**: 
- Verificar contratos associados ao encarregado na página de Contratos
- Desassociar encarregado dos contratos primeiro
- Depois tentar excluir novamente

### Problema: Contrato aparece como "(Sem Contrato)" mas deveria aparecer
**Causa**: Possíveis causas:
1. `ContratoId` do encarregado está vazio ou inválido
2. JOIN falhou (contrato não existe ou foi excluído)
3. Fornecedor do contrato não existe

**Solução**:
- Verificar se o encarregado tem `ContratoId` válido no banco
...
**Causa**: Possíveis causas:
1. Endpoint `/api/encarregado` retornou erro
...
- **JOINs Complexos**: A listagem usa múltiplos LEFT JOINs para incluir informações de Contrato e Fornecedor, permitindo encarregados sem contrato
- **Validação de Dependências**: A exclusão de encarregado valida se existem contratos associados antes de permitir a exclusão, evitando inconsistências no banco de dados
...
- **⚠️ BUG CONHECIDO**: O método `UpdateStatusEncarregado` não chama `Save()` após `Update()`, o que pode impedir a persistência da alteração de status
...
**Descrição**: Documentação completamente reescrita seguindo o padrão "Problema → Solução → Código", com fluxos detalhados, tabela de endpoints e troubleshooting expandido. Incluída documentação do modal de foto e dos JOINs complexos. Incluída nota sobre bug conhecido no método `UpdateStatusEncarregado`.
...
**Descrição**: Documentação inicial da listagem de Encarregados (Index).
