...
(function (root) {
  ...
  if (!$) return; // sem jQuery, não faz nada

  // evita redefinir se já existir
  if ($.throttle && $.debounce) return;
  ...
  function throttle(delay, no_trailing, callback, debounce_mode) {
    ...
    function wrapper() {
      var self = this, elapsed = Date.now() - last_exec, args = arguments;
      ...
      if (debounce_mode === undefined && elapsed > delay) {
        ...
        timeout_id = setTimeout(debounce_mode ? function () {
          timeout_id = void 0;
        } : function () {
          exec();
          timeout_id = void 0;
